...
# 🎪 البرومبت المحسن لشخصية المهرج المزوح (لضمان الفكاهة في كل رد)
DYNAMIC_PROMPT = """
...
# (تم تبسيطها للتوافق مع النموذج الأصلي لكن مع الجواب الواضح)
FUNNY_PUZZLES = {
    "رياضة": [
...
def initialize_user_session(user_id):
    ...
    if user_id not in chat_sessions:
        # تخزين بيانات اللغز النشط والجواب الصحيح في الذاكرة
...
def understand_user_intent(message):
    ...
    if any(word in message_lower for word in ['لغز', 'جديد', 'اريد لغز', 'تحدي']):
        ...
        return 'request_help' # ميزة التلميح المجنون
...
def generate_puzzle_data(category, level, user_id):
    ...
    # 💡 محاولة استخراج الجواب من Gemini (لضمان ألغاز جديدة في كل مرة)
    if GEMINI_API_KEY:
        try:
    ...
    return "🎲 ما الذي ينام ويقظ لكنه لا يتعب؟", "السرير"
...
def evaluate_and_reply_with_gemini(user_id, user_attempt, current_puzzle, correct_answer):
    ...
    **توجيهات الرد:**
    ...
    - يجب أن يكون الرد لاذعاً ومضحكاً ولا يتجاوز سطرين.
    """
    
    # منطق احتياطي للتقييم بدون API
    ...
    try:
        ...
        if full_text.startswith('[صحيح]'):
            ...
            # في حال لم يتبع النموذج التنسيق، نعتبره خطأ مع رد مرح
...
@app.route('/')
def serve_html():
    """خدمة ملف HTML (LUKU-AI.html) - للحفاظ على الواجهة الأصلية"""
    try:
        # تأكد من أن هذا يتطابق مع اسم ملف الواجهة الخاص بك
        with open('LUKU-AI.html', 'r', encoding='utf-8') as file:
            return file.read()
    except Exception as e:
        return f"<html><body><h1>خطأ: لم يتم العثور على LUKU-AI.html</h1><p>يرجى التأكد من وجود ملف الواجهة الخاص بك.</p></body></html>"
    ...
    try:
        if is_first_message or user_intent == 'request_puzzle':
            ...
            if current_puzzle:
                ...
                كن مرحاً جداً. ابدأ ردك بـ "تلميح مجنون 🤯:"
                """
                if GEMINI_API_KEY:
                    model = genai.GenerativeModel('gemini-1.5-flash')
            ...
            if not current_session.get('current_puzzle'):
                ...
                # 3. تحديث النقاط (Gamification المؤقتة)
                if is_correct:
...
if __name__ == '__main__':
    # تأكد من إضافة `google-genai` في requirements.txt
