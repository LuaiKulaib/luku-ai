...
def home():
    ...
    <html>
    <head>
        <title>LUKU AI - المساعد الذكي</title>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
    </head>
    <body style="background: #0b0e14; color: white; text-align: center; padding: 50px; font-family: Arial, sans-serif;">
        <h1 style="font-size: 2.5rem; margin-bottom: 20px;">🧩 LUKU AI</h1>
        <p style="font-size: 1.2rem; margin-bottom: 30px;">المساعد الذكي للألغاز والتحديات الفكرية</p>
        <div style="background: #1a1f2e; padding: 20px; border-radius: 10px; display: inline-block;">
            <p>✅ الخادم يعمل بشكل صحيح</p>
            <p>🎯 التوليد الديناميكي: 100% بواسطة الذكاء الاصطناعي</p>
            <p>🚀 لا توجد ألغاز مخزنة مسبقاً</p>
        </div>
    </body>
    </html>
    """
...
if __name__ == '__main__':
    port = int(os.environ.get("PORT", 3000))
    print(f"🚀 بدء تشغيل LUKU AI على المنفذ {port}")
    print(f"🎯 الميزات: توليد كامل ديناميكي - لا توجد ألغاز مخزنة")
    print(f"🧠 المحتوى: 100% يتم إنشاؤه بواسطة الذكاء الاصطناعي")
    app.run(host='0.0.0.0', port=port, debug=False)
